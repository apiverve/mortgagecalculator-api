...
class MortgagecalculatorAPIClientError(Exception):
...
class ValidationError(MortgagecalculatorAPIClientError):
...
class MortgagecalculatorAPIClient:
    ...
    VALIDATION_RULES = {"amount": {"type": "number", "required": True, "min": 0}, "rate": {"type": "number", "required": True, "min": 0, "max": 100}, "years": {"type": "integer", "required": True, "min": 1, "max": 50}, "downpayment": {"type": "number", "required": False, "min": 0}, "annual_propertytax": {"type": "number", "required": False, "min": 0}, "annual_homeinsurance": {"type": "number", "required": False, "min": 0}, "annual_hoa": {"type": "number", "required": False, "min": 0}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the MortgagecalculatorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises MortgagecalculatorAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise MortgagecalculatorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[MortgagecalculatorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns MortgageCalculatorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises MortgagecalculatorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise MortgagecalculatorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise MortgagecalculatorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise MortgagecalculatorAPIClientError(f"Request failed: {str(e)}")
